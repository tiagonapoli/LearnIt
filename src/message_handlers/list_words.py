...
def handle_list_words(bot, rtd, debug_mode):	
	...
	def list_words(msg):
		...
		if len(known_languages) == 0:
			bot.send_message(user_id, bot_language.translate("_Please, add a language first._", user),
				parse_mode="Markdown")
	...
	def list_words1(msg):
		...
		if len(topics) > 0:
			...
			bot.send_message(user_id, 
				bot_language.translate("_There are no topics registered in this language yet._", user), parse_mode="Markdown")
	...
	def list_words2(msg):
		...
		if valid == False:
			bot.reply_to(msg, 
				bot_language.translate("*Please choose from options.*", user), 
				parse_mode="Markdown")
		...
		markup = bot_utils.keyboard_remove()
		text = (bot_language.translate("*Select the word you want to see the related media* _(to exit this menu, select_ /{}_)_:",user).format(len(words_string))
	...
	def list_words3(msg):
		...
		if valid == False:
			bot.reply_to(msg, 
				bot_language.translate("*Please choose from options.*", user), 
				parse_mode="Markdown")
			text = (bot_language.translate("*Select the word you want to see the related media* _(to exit this menu, select_ /{}_)_:", user).format(len(user.keyboard_options))
		...
		bot.send_message(user_id, 
			bot_language.translate("*Word:* _{}_", user).format(utils.treat_msg_to_send(utils.get_foreign_word(user.temp_words_list[option]), "_")), 
				parse_mode="Markdown")
		utils.send_all_cards(bot, user.temp_words_list[option], user)
